...
首版已上线：`shadow-loom.html`，6 幕覆盖全部五章（Prototype 的兔 / 鸟 / 鹿或树之外补了鲸与鹤）。

| 幕 | 章 | 纸片 | 机制 |
| --- | --- | --- | --- |
| 兔 | 初影 | 3（2 个深度） | 灯固定、只能拖纸片 |
| 飞鸟、鲸 | 错层 | 3（3 个深度） | 同样的拖动，影子走的距离不同 |
| 鹿 | 灯行 | 3，鹿角钉住 | 拖灯；钉住的纸片只能靠灯对准 |
| 树 | 回旋 | 4，树干钉住 | 拖灯 + 拖铜钮旋转 |
| 鹤 | 活影 | 5，身体钉住 | 目标只在开场与轻触顶栏图标时浮现 |

实现要点：

- **同源投影**：`js/shadow-loom-rules.js` 是渲染、判定、校验器共用的唯一几何内核（§6.1 公式原样实现）。
  纸片形状按「影子尺度」定义，纸片本体 = 影子 × z；关卡的解写成影子中心，纸片摆位由公式反推，构造即可解。
  悬挂晃动（松手后的惯性）作为绕悬挂点的转动同时进入渲染与判定。
...
- **完成奖励**：纸片与灯收紧到精确解（0.45s）→ 金线沿完整轮廓扫一圈、周边暗下（0.8s）→ 活影动作（兔抖耳跳两步、鸟振翅飞走、
  鲸摆尾、鹿抬头、树随风摇并落叶、鹤展翅升起）→ 结果层停在舞台下部，记录用时 / 移动 / 最高匹配 / 一次完成。
