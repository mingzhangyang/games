...
- **Shadow Loom 影织** (`shadow-loom.html`) - Paper-theatre shadow puzzle (design: `docs/proposal-shadow-loom-design.md`, art refs `assets/shadow-loom/`): drag paper cut-outs hung at different depths (and, from chapter 3, the lamp) until their shadows weave into the target silhouette. One projection model `S = L + (P − L) / z` in `js/shadow-loom-rules.js` feeds both rendering and judging (5px mask grid, IoU + per-piece contour score, thresholds in `THRESHOLDS`); level solutions are written as shadow centres so every level is solvable by construction (`js/shadow-loom-levels.js`, checked by `scripts/verify-shadow-loom-levels.mjs`). Dark-only; no leaderboard; progress in `sl_progress`
