// tank-entities.js — 坦克大战实体层（自 tank-battle.js 抽出，move-only）。
// CONFIG/WEAPONS 为实体与主类共享的物理/武器配置；5 个实体类自包含，
// 仅经 game 参数反向访问主类运行时状态（particles/bullets/enemies/walls/player）。
// i18n：PowerUp.getIcon 需要翻译函数，主文件加载后调用 bindTankI18n(t) 注入
// （t 缺 key 时返回 key 本身，未注入时的直通实现与之等价）。

let translateText = null;
export function bindTankI18n(fn) {
    translateText = fn;
}
const tt = (key) => (translateText ? translateText(key) : key);
...
export class PowerUp {
    ...
    getIcon() {
        return tt(`powerUpIcons.${this.type}`) || '?';
    }
}
